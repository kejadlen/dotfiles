...
8. **Code review**: Review the diff from trunk to the revision before opening the
   ...
   directory, which may be on a different revision. `--git` gives a standard
   unified diff with `@@` hunk headers; the default color-words format prints two
   line-number columns (old, then new) and shows `..` in the old column for added
   lines, which is easy to misread when anchoring `file:line`.
9. Create the PR: `gh pr create --head <bookmark-name> --title "<title>"` using
...
10. **If a Jira card is detected**, transition it to "In Review":
